...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Error in whitebox defense evaluation proxy:', error)
    ...
    return NextResponse.json(
      { error: 'Failed to process whitebox defense evaluation', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    )
  }
}
